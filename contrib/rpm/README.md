...
The RPM spec file provided here is for Rogue-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://rogue.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Rogue git repository](https://github.com/rogue/rogue) but are not part of
...
Currently only `Source10` is used, for the example `rogue.conf` file.
...
in the rogue git repository. At present only an SVG version of the Rogue
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the rogue
...
needed when 0.12.1 is released, a proper fix is already in the Rogue git
master branch.
...
idea to build Rogue Core against a static version of that library compiled
...
The `rogue.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `rogue-server` package. At this point, most RPM based
...
The `rogue-server` package should have SELinux support. How to properly do
...
Rogue-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/rogue/). He has been
packaging Rogue for Fedora at least since 2012.
...
4. I renamed the `rogue` package that contains the Qt GUI to `rogue-core` as
that appears to be how the general population refers to it, in contrast to
`rogue-xt` or `rogue-classic`. I wanted to make sure the general population

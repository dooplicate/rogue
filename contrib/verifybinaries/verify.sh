...
###   roguecore.org and rogue.org and compares them.
...
WORKINGDIR="/tmp/rogue_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://roguecore.org"
HOST2="https://rogue.org"
BASEDIR="/bin/"
VERSIONPREFIX="rogue-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'rogue-',
...
if [ $? -ne 0 ]; then
   echo "rogue.org failed to provide signature file, but roguecore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "rogue.org and roguecore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Rogue Core binary release signing key installed?"
   fi

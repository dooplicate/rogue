...
class AbandonConflictTest(RogueTestFramework):
    ...
    def run_test(self):
        ...
        # Verify that B and C's 10 ROGUE outputs are available for spending again because AB1 is now conflicted
        newbalance = self.nodes[0].getbalance()
        assert_equal(newbalance, balance + Decimal("20"))
        balance = newbalance

        # There is currently a minor bug around this and so this test doesn't work.  See Issue #7315
        # Invalidate the block with the double spend and B's 10 ROGUE output should no longer be available

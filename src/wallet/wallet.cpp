...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CRogueAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/rogue/rogue/pull/10340, but that change has been
    // postponed until after https://github.com/rogue/rogue/pull/10244 to
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-rogue-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CRogueAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CRogueAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CRogueAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CRogueAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CRogueAddress(address).ToString());
}
 ...
 * https://roguetalk.org/?topic=54527, or
 * https://github.com/rogue/rogue/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CRogueAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CRogueAddress(dest).ToString(), key);
}

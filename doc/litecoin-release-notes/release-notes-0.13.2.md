Rogue Core version 0.13.2 is now available from:

  <https://download.rogue.org/rogue-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a rogue
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of Rogue Core
randomly crashing on Windows XP. It is [not clear](https://github.com/rogue/rogue/issues/7681#issuecomment-217439891)
...
ECDSA signatures inside Rogue transactions now use validation using
[libsecp256k1](https://github.com/rogue-core/secp256k1) instead of OpenSSL.
...
(https://github.com/rogue/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Rogue Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Rogue Core was to raise the default minimum
relay fee.

Rogue Core 0.13.2 will have a strict maximum size on the mempool. The
...
Rogue Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running rogued without having to do any manual configuration.
...
Rogue Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Rogue Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Rogue Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Rogue Core 0.13, and it is at this time undecided whether the
...
Rogue Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Rogue Core automatically creates a hidden service to listen on, without
manual configuration. Rogue Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Rogue Core is listening, and
...
Rogued can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Rogue
Core to use floating fees. By default, Rogue Core will use floating
fees.
...
(default: `0.0002` ROGUE/kB).

At all times, Rogue Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ROGUE.
Furthermore, Rogue Core will never create transactions paying less than
...
`rogue.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of Rogue Core v0.11.0](https://github.com/rogue/rogue/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/rogue/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/rogue/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /roguerpc>
        ...
        # optional bypass rogued rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< roguerpc:<password>
    </Location>
...
Restarting rogued will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in rogue.conf
...
For example:
    ...
    $ src/rogue-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Rogue Core code base has
...
- `rogue-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `rogue-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
<https://github.com/rogue/rogue/pull/7184>

BIP112 soft fork to enforce OP_CHECKSEQUENCEVERIFY
--------------------------------------------------

[BIP112][] redefines the existing OP_NOP3 as OP_CHECKSEQUENCEVERIFY (CSV)
for a new opcode in the Rogue scripting system that in combination with
[BIP68][] allows execution pathways of a script to be restricted based
on the age of the output being spent.

For more information about the implementation, see
<https://github.com/rogue/rogue/pull/7524>
...
Rogue transactions currently may specify a locktime indicating when
...
For more information about the implementation, see
<https://github.com/rogue/rogue/pull/6566>
...
compatible peers.
[BIP 152](https://github.com/rogue/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of Rogue Core.

[Pull request](https://github.com/rogue/rogue/pull/8035/files), [BIP 32](https://github.com/rogue/bips/blob/master/bip-0032.mediawiki)
...
Witness)] (https://github.com/rogue/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/rogue/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the Rogue-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/rogue/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/rogue/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP
112](https://github.com/rogue/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/rogue/rogue/pull/7762).
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Rogue wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Rogue block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of rogues they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of rogues being spent is exactly
...
- **Increased security for multisig:** Rogue addresses (both P2PKH addresses
  ...
  believed to be provided by Rogue's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Rogue paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Rogue's current ledger (called the UTXO set),
  ...
  note that Rogue Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Rogue users to individually opt-in to almost any change in the Rogue
  Script language when those users receive new transactions.  Features
  currently being researched by Rogue and Rogue Core contributors that may
  ...
  Other Rogue community members are studying several other improvements
...
segwit's start date of 1 January 2017 miners can update the Rogue
client to Rogue Core 0.13.2 to signal for segwit support. When a
...
[Segwit FAQ]: https://roguecore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://roguecore.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/rogue/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/rogue/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/rogue/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/rogue/bips/blob/master/bip-0145.mediawiki
...
Since Rogue Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/rogue/bips/blob/master/bip-0147.mediawiki
...
- `rogue-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `rogue-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting

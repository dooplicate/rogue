Rogue Core version *0.15.1* is now available from:

  <https://download.rogue.org/rogue-0.15.1/>
...
A number of changes to the way Rogue Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 rogues per kB (0.00001 ROGUE). This is to prepare for dropping the minimum transaction fee to 0.1 rogues per kB (0.0001 ROGUE) in the next release.
...
- #11433 `e169349` Restore rogue-util-test py2 compatibility (MarcoFalke)
- #11308 `2e1ac70` zapwallettxes: Wait up to 3s for mempool reload (MarcoFalke)
- #10798 `716066d` test rogue-cli (jnewbery)
...
- #11125 `812c870` Add rogue-cli -stdin and -stdinrpcpass functional tests (promag)
- #11077 `1d80d1e` fix timeout issues from TestNode (jnewbery)
- #11078 `f1ced0d` Make p2p-leaktests.py more robust (jnewbery)
- #11210 `f3f7891` Stop test_rogue-qt touching ~/.rogue (MeshCollider)
...
### Miscellaneous
- #11377 `75997c3` Disallow uncompressed pubkeys in rogue-tx [multisig] output adds (TheBlueMatt)

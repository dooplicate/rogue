Rogue Core version 0.12.0 is now available from:

  <https://rogue.org/bin/rogue-core-0.12.0/>
...
pre-0.12 versions of Rogue Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Rogue Core version 0.11 or
earlier.
...
Rogue Core 0.12 will have a strict maximum size on the mempool. The
...
Rogue Core 0.12 also introduces new default policy limits on the length and
...
Rogue Core 0.12 nodes. Rogue Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/rogue/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Rogue Core 0.12 does not yet have support for
...
notes of v0.11.0](https://github.com/rogue/rogue/blob/v0.11.0/doc/release-notes.md#block-file-pruning).
...
- #6214 `87406aa` [OSX] revert renaming of Rogue-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new rogue-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include rogue-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix rogue-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of rogue-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [rogue-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BITCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library librogueconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "ROGUE" (Ross Nicoll)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/rogue/).

...
*Setup instructions for a Gitian build of Rogue Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Rogue
...
- [Building Rogue Core](#building-rogue-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-rogue-host/README.md).
...
Clone the git repositories for rogue and Gitian.
...
in the rogue repository under 'Fetch and create inputs' to install sources which require
...
To build Rogue Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the rogue repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/rogue/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/rogue-project/rogue
...
For example:
```bash
URL=https://github.com/thrasher-/rogue.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit rogue=${COMMIT} --url rogue=${URL} ../rogue/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit rogue=${COMMIT} --url rogue=${URL} ../rogue/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit rogue=${COMMIT} --url rogue=${URL} ../rogue/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the rogue git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../rogue/contrib/gitian-descriptors/*|sort|uniq )
...
ROGUEPATH=/some/root/path/rogue
SIGPATH=/some/root/path/rogue-detached-sigs

./bin/gbuild --url rogue=${ROGUEPATH},signature=${SIGPATH} ../rogue/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/rogue-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/rogue-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/rogue-osx-build.assert
```
...
[rogue-project/gitian.sigs.ltc](https://github.com/rogue-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
